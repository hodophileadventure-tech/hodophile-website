...
function getCustomChilasMinimumDays(city: string, startingPointValue: string) {
  if (city === "Islamabad") {
    return 2;
  }

  if (city === "Chilas (Arrival)" || city === "Chilas (Return)") {
    return 1;
  }
...
export function MakeMyTripForm() {
  ...
  const isFixedPackageSelection = Boolean(selectedLuxuryPackage || selectedHoneymoonTour || selectedPreplannedTrip);

  const usesChilasItinerary = isCustomCitySelection() && !isFixedPackageSelection && travelMode !== "air" && selectedCities.some((c) => CHILLAS_GROUP.has(c));
  ...
  const shouldAutoIncludeIslamabad = (): boolean => {
    ...
    if (travelMode === "air") return false;
  ...
  useEffect(() => {
    const key = toCombinationKey(selectedCities.filter((c) => c && c.trim() !== ""));
    const combo = COMBO_ITINERARIES[key];
    if (!combo) return;

    // compute effective total nights (days - 1), adjust for Islamabad start
    const startReduction = getActualStartingPoint().toLowerCase().trim() === "islamabad" ? (combo.reduceIfStartsIslamabad || 0) : 0;
    const totalDays = Math.max(1, combo.totalDays - startReduction);
    const totalNights = Math.max(1, totalDays - 1);

    // Distribute nights across the selected cities (exclude Islamabad/Chilas markers)
    const cities = selectedCities.filter((c) => c && c.trim() !== "" && !c.startsWith("Chilas") && c !== "Islamabad");
    ...
    for (const city of cities) {
      nights[city] = base + (rem > 0 ? 1 : 0);
      rem = Math.max(0, rem - 1);
    }

    setCustomCityNights((prev) => ({ ...prev, ...nights }));
  }, [selectedCities, startingPoint, otherStartingPoint]);

  // Off-route combinations are blocked; keep charge at 0
  useEffect(() => {
    setOffRouteChargePKR(0);
  }, [selectedCities, vehicleName, isInvalidCombination]);
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      if (isInvalidCombination) {
        setSubmitError("Please select an on-route city combination.");
        setIsSubmitting(false);
        return;
      }
      ...
      // Augment multi-city hotels/nights with Chilas only for true custom itineraries.
      // Fixed package routes already define their itinerary and should not inherit Chilas rows.
      const shouldIncludeMultiCityHotels = isCustomCitySelection() && !supportsMultipleHotelsInCustomSingleCity;
        ...
        <div className="relative overflow-hidden rounded-[30px] bg-gradient-to-br from-[#fff6d6] via-white to-[#fff1bd] p-[1.5px] shadow-[0_32px_80px_rgba(0,0,0,0.12)] transition-all duration-500">
                    ...
                    <div className="py-2">
                      {orderedFeaturedTourCards.map((tour) => (
                        <button
                          ...
                          className="block w-full px-4 py-3 text-left text-sm font-semibold text-black hover:bg-white transition"
                        >
                          {tour.title}
                        </button>
                      ))}
          ...
          <form onSubmit={handleSubmit} className="grid gap-4">
            ...
            <div className="grid grid-cols-1 gap-4 rounded-[24px] border border-[#E8A500] bg-white p-4 shadow-[0_8px_20px_rgba(0,0,0,0.06)] lg:grid-cols-2">
              <label className="grid gap-2 text-sm font-medium text-black">
                ...
                <input
                  ...
                  className="glow-focus rounded-[15px] border border-[#DDD] bg-white px-4 py-3 text-sm text-black outline-none transition placeholder:text-stone-500 focus:border-[#FCC000] focus:ring-4 focus:ring-[#FCC000]/20"
                />
              </label>
              <label className="grid gap-2 text-sm font-medium text-black">
                <span className="flex items-center gap-2">
                ...
                <input
                  ...
                  placeholder="+92..."
                  className="glow-focus rounded-[15px] border border-[#f4d77d] bg-white px-4 py-3 text-sm text-black outline-none transition placeholder:text-stone-400 focus:border-[#fcc000] focus:ring-4 focus:ring-[#fcc000]/15"
                />
              </label>
            </div>
            ...
            <div className="grid gap-4 rounded-[24px] border border-[#f4d77d] bg-white p-4 shadow-[0_8px_20px_rgba(252,192,0,0.06)]">
              <div className="grid grid-cols-1 gap-4 lg:grid-cols-2">
                <label className="grid gap-2 text-sm font-medium text-black">
                  ...
                  <input
                    ...
                    className="glow-focus rounded-[15px] border border-[#f4d77d] bg-white px-4 py-3 text-sm text-black outline-none transition focus:border-[#fcc000] focus:ring-4 focus:ring-[#fcc000]/15"
                  />
                </label>
                ...
                <label className="grid min-w-0 gap-2 text-sm font-medium text-black">
                  ...
                  <select
                    ...
                    className="glow-focus w-full min-w-0 rounded-[15px] border border-[#f4d77d] bg-white px-4 py-3 text-sm text-black outline-none transition focus:border-[#fcc000] focus:ring-4 focus:ring-[#fcc000]/15 disabled:cursor-not-allowed disabled:bg-[#f8efc8] disabled:text-stone-400 disabled:opacity-70"
                  >
              ...
              {startingPoint === "Other" && (
                <label className="grid gap-2 text-sm font-medium text-black lg:col-span-2">
                  Enter Starting City *
                  <input
                    required
                    type="text"
              ...
              <div className="rounded-[14px] bg-white border border-[#f4d77d] p-3 mb-4 flex items-center justify-between">
                ...
                <div>
                  <button
                    ...
                    className="rounded-full px-3 py-1 text-sm font-semibold text-[#8a4b00] border border-[#8a4b00] bg-white hover:bg-[#fff3db]"
                  >
                    Remove
                  </button>
                </div>
            ...
            <div className={`rounded-[20px] border border-[#f4d77d] bg-white p-4 shadow-[0_8px_20px_rgba(252,192,0,0.06)] ${chosenPackage ? 'hidden' : ''}`}>
              ...
              {effectiveSelectedCities.length > 0 && (
                <div className="mt-3 space-y-2 text-xs text-[#6e5200]">
                  ...
                  {shouldAutoIncludeIslamabad() && (
                    <div className="mt-2 rounded-[14px] bg-[#fff2c8] border border-[#f4d77d] p-3 text-sm text-black flex flex-wrap items-start justify-between gap-3">
            ...
            {isPackageRoute() && !isMultiCityTour() ? (
              <div className="grid gap-4 grid-cols-1 xl:grid-cols-2">
                ...
                <label className="grid gap-2 text-sm font-medium text-black min-w-0 z-50 overflow-hidden">
                  ...
                  <select
                    ...
                    disabled={!hotelId}
                    className="rounded-[15px] border border-[#f4d77d] bg-white px-4 py-3 text-sm text-black outline-none transition focus:border-[#fcc000] focus:ring-4 focus:ring-[#fcc000]/15 disabled:bg-[#f8efc8] disabled:text-stone-500 appearance-none w-full overflow-hidden text-ellipsis"
                  >
                    <option value="">
              ...
              <div className="rounded-[15px] border border-[#f4d77d] bg-white p-4">
                ...
                {getVisibleMultiCityConfig(routeId)?.cities.map((city) => {
                  ...
                  return (
                    <div
                      key={city}
                      className="mb-4 pb-4 border-b border-[#f4d77d] last:border-b-0 rounded-[10px] bg-white p-3"
                    >
              ...
              <div className="rounded-[15px] border border-[#f4d77d] bg-white p-4">
                <div className="mb-4 flex flex-wrap items-start justify-between gap-3">
                  ...
                  <div className="rounded-full border border-[#f4d77d] bg-white px-3 py-1 text-xs font-semibold text-[#6e5200]">
                ...
                <div className="space-y-4">
                  {singleCityHotelStays.map((stay, index) => {
                    ...
                    return (
                      <div key={`${stay.hotelId}-${index}`} className="rounded-[10px] border border-[#f4d77d] bg-white p-3 shadow-sm">
                ...
                <div className="mt-4 flex items-center justify-between gap-3">
                  <button
                    ...
                    className="rounded-[10px] border border-[#f4d77d] bg-white px-4 py-2 text-sm font-semibold text-[#6e5200] hover:bg-[#fffdf3]"
                  >
                    + Add Another Hotel
                  </button>
            ...
            {totalGuests > 4 && (
              <div className="rounded-[18px] bg-white border border-[#f4d77d] p-4 text-sm text-[#6e5200] shadow-[0_8px_18px_rgba(252,192,0,0.06)]">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-3">
                  {Array.from({ length: kids }).map((_, index) => (
                    <input
                      ...
                      className="rounded-[15px] border border-[#f4d77d] bg-white px-4 py-3 text-sm text-black outline-none transition placeholder:text-stone-400 focus:border-[#fcc000] focus:ring-4 focus:ring-[#fcc000]/15"
                    />
                  ))}
                </div>
              </div>
